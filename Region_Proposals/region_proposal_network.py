# Got help from https://github.com/hxuaj/tf2-faster-rcnn/blob/main/model/rpn.py
# Region Proposal Network

import tensorflow as tf
from tensorflow.keras import layers
from Utils import generate_anchors
from Region_Proposals import proposal_layer
...
class RPN(tf.keras.Model):
	...
	def __init__(self, is_training, anchor_scales = [8, 16, 32], anchor_ratios = [0.5, 1, 2], feat_stride = cfg.feat_stride):
		super().__init__()

		# Arguments
		self.is_training = is_training
		self.num_anchors = tf.cast(len(anchor_scales) * len(anchor_ratios), dtype = tf.int32) # Basically number of anchors is 9
		self.feat_stride = tf.constant(feat_stride, dtype = tf.int32)
		self.anchors_ref = generate_anchors.generate_anchor_ref(scales = anchor_scales, ratios = anchor_ratios)

		# Layers
		self.rpn_conv = layers.Conv2D(512, (3, 3), padding = 'same', activation = 'relu', name = 'rpn_conv')
		self.score_conv = layers.Conv2D(self.num_anchors * 2, (1, 1), activation = None, name = 'score_conv')
		self.bbox_conv = layers.Conv2D(self.num_anchors * 4, (1, 1), activation = None, name = 'bbox_conv')
		self.softmax = layers.Softmax(axis = -1)
		self.proposal_layer = proposal_layer.proposal_layer(self.is_training)
	...
	def call(self, feature_map, img_size, gt_boxes=None, gt_cls=None):
		...
		(anchors, rpn_cls_prob, self.rpn_cls_score, self.rpn_bbox_pred, h, w) = self.rpn_body(feature_map)
		...
		if self.is_training:
			...
			rois = self.proposal_layer(rpn_cls_prob, self.rpn_bbox_pred, anchors, img_size)
	...
	def rpn_body(self, feature_map):
		...
		anchors = generate_anchors.generate_anchors(self.anchors_ref, self.feat_stride, h, w)
