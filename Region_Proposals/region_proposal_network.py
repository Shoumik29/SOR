...
from Utils import generate_anchors, anchor_to_target
...
class RPN(tf.keras.Model):
    ...
    def __init__(self, is_training, anchor_scales=[8, 16, 32], anchor_ratios=[0.5, 1, 2], 
        ...
        # Layers:
        self.rpn_conv = layers.Conv2D(512, (3, 3), kernel_regularizer=self.regularizer, activation='relu', 
                               kernel_initializer=self.initializer, padding='same', name='rpn_conv')
        self.score_conv = layers.Conv2D(self.num_anchors * 2, (1, 1), kernel_regularizer=self.regularizer,
                                 activation=None, kernel_initializer=self.initializer, name='score_conv')
        self.bbox_conv = layers.Conv2D(self.num_anchors * 4, (1, 1), kernel_regularizer=self.regularizer,
    ...
    def call(self, feature_map, img_size, gt_boxes=None, gt_cls=None):
        ...
        if self.is_training:
            # anchors_to_target
            rpn_labels, rpn_bbox_targets = anchor_to_target.anchor_to_target(h, w, self.num_anchors, 
    ...
    def rpn_body(self, feature_map):
        ...
        print("--------Shoumik---------", rpn.shape)
        rpn_cls_score = self.score_conv(rpn)
        print("asdsdsaf", rpn_cls_score.shape)
